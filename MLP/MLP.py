...
def scheduler(epoch):
    ...
    if epoch % 4 == 0 and epoch != 0:
        ...
        K.set_value(model.optimizer.lr, lr * 0.005)
        print("lr changed to {}".format(lr * 0.005))
...
if __name__ == '__main__':
    ...
    AllNodeBehavior1 = []
    ReadMyCsv1(AllNodeBehavior1, 'fused_features_7_1500.csv')

    # ---------划分训练-------

    PositiveSample_Train = []
    ReadMyCsv1(PositiveSample_Train, 'PositiveSample_Train_肝癌.csv')
    PositiveSample_Validation = []
    ReadMyCsv1(PositiveSample_Validation, 'PositiveSample_Validation_肝癌.csv')
    PositiveSample_Test = []
    ReadMyCsv1(PositiveSample_Test, 'PositiveSample_Test_肝癌.csv')

    NegativeSample_Train = []
    ReadMyCsv1(NegativeSample_Train, 'NegativeSample_Train_肝癌.csv')
    NegativeSample_Validation = []
    ReadMyCsv1(NegativeSample_Validation, 'NegativeSample_Validation_肝癌.csv')
    NegativeSample_Test = []
    ReadMyCsv1(NegativeSample_Test, 'NegativeSample_Test_肝癌.csv')
    ...
    while CounterT < 5:
        ...
        x1 = Dense(64, activation='relu', activity_regularizer=regularizers.l2(0.0001))(input1)

        # ——输入2——
        input2 = Input(shape=(len(x_train_2_Behavior1[0]),), name='input2')
        x2 = Dense(128, activation='relu', activity_regularizer=regularizers.l2(0.0001))(input2)
        ...
        hidden = Dropout(rate=0.4)(hidden)
        hidden = Dense(32, activation='relu', name='hidden3', activity_regularizer=regularizers.l2(0.0001))(hidden)
        ...
        while counter < len(ModelTestOutput):
            ...
            counter = counter + 1
        LabelPredictionProbName = '肝癌_RealAndPredictionProbA+B' + str(CounterT) + '.csv'
        StorFile(LabelPredictionProb, LabelPredictionProbName)
        LabelPredictionName = '肝癌_RealAndPredictionA+B' + str(CounterT) + '.csv'
